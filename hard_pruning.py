...
def main(args):
    ...
    if str.lower(model_dataset) == 'cifar100':
        num_classes = 100
    else:
        return ValueError

    model_name = get_model_name(model_path=model_path)
    model = model_dict[model_name[0]](num_classes=num_classes)
    ...
    pruning_time = round((end_pruning - start_pruning).total_seconds(), 3)
    print(f'pruned MACs: {macs/1e6:.2f} M \t pruned params: {nparams/1e6:.2f} M')
    print(f'Pruning time: {pruning_time}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, required=True, choices=['cifar100'])
